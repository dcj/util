(ns util.http.status
  "Tools to deal with http status responses"
...
(defn in-range?
  ...
  (= digit (range http-response-code)))

(def code->name-description-map
  "Map of status code to map containing :name and :description"
  {
   100 {:name "Continue"
...
(defn get-name
  ...
  [code]
  (get-in code->name-description-map [code :name]))

(defn get-description
  "Returns http status description by code."
  [code]
  (get-in code->name-description-map [code :description]))
